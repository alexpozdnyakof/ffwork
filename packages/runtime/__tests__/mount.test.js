...
test("should mount an element without key attribute", () => {
  const node = h("button", { key: "el" }, [hString("submit")]);
  mount(node, document.body);
  expect(node.props.key).toBe("el");
  expect(document.body.innerHTML).toBe(`<button>submit</button>`);
});
...
test("should mount an element with multiple classNames", () => {
  const node = h("button", { class: ["submit-btn", "primary", "outlined"] }, [
    hString("submit"),
  ]);
  mount(node, document.body);
  expect(document.body.innerHTML).toBe(
    `<button class="submit-btn primary outlined">submit</button>`
  );
});

test("should mount an element with style", () => {
  const node = h("button", { style: { fontWeight: "bold" } }, [
    hString("submit"),
  ]);
  mount(node, document.body);
  expect(document.body.innerHTML).toBe(
...
test("should change css custom property value", () => {
  const css =
    ":root { --text-color: blue; }; .btn { color: var(--text-color) }; ";
  ...
  node = h("button", { class: "btn", style: { "--text-color": "red" } }, [
    hString("submit"),
  ]);
  mount(node, document.body);
...
test("should mount nested fragments recursively", () => {
  const node = hFragment([
    h("p", {}, ["first line"]),
    h("p", {}, ["second line"]),
    hFragment([h("p", {}, ["third line"]), h("p", {}, ["fourth line"])]),
  ]);
  mount(node, document.body);
  expect(document.body.innerHTML).toBe(
